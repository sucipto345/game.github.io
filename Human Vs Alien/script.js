...
function chooseDefender(){
  ...
  ctx.drawImage(peaSeed, 0, 0, 194, 194, 10, 5, 245, 245);
  ctx.fillRect(card2.x, card2.y, card2.width, card2.height);
  ctx.drawImage(icePeaSeed, 0, 0, 194, 194, 90, 5, 245, 245);
...
function handleResources(){
  ...
  for (let i = 0; i < resources.length; i++){
    ...
    if (resources[i] && mouse.x && mouse.y && collision(resources[i], mouse)){
      ...
      floatingMessages.push(new FloatingMessages('+' + resources[i].amouts, resources[i].x, resources[i].y, 30, 'black'));
      floatingMessages.push(new FloatingMessages('+' + resources[i].amouts, 250, 50, 30, 'gold'));
...
canvas.addEventListener('click', function(){
  ...
  if (numberOfResources >= defenderCost){
    ...
    floatingMessages.push(new FloatingMessages('need more resources', mouse.x, mouse.y, 15, 'blue'));
  }
});
